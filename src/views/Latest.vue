<template>
    <div class="container">
        <h1 class="title">Latest posts - page {{ currentPage }}</h1>
...
export default {
    ...
    methods: {
        getLatestProducts() {
            djangoAPI
                .get(`/api/v1/products/latest/?page=${this.$route.params.page}`)
                .then((latestResponse) => {
                    console.log(latestResponse);
                    this.products = latestResponse.data.results;
                    this.lastPage = Math.ceil(latestResponse.data.count / 21);
        ...
        selectPage(page) {
            djangoAPI
                .get(`/api/v1/products/latest/?page=${page}`)
        ...
        setTitle() {
            document.title = `Latest Posts - Page ${this.$route.params.page} | MechMarketEU`;
        },
    },
    created() {
